...
interface YouTubeMusicShortsSongCardProps {
...
function buildPreviousRankTag(item: YouTubeMusicChartItem, locale: Locale): YouTubeVideoCardTag {
...
function buildDaysTag(item: YouTubeMusicChartItem, locale: Locale): YouTubeVideoCardTag | null {
  const t = getMessages(locale).youtubeMusicShortsDaily;
...
function formatViewsText(
  ...
  locale: Locale,
  t: ReturnType<typeof getMessages>['youtubeMusicShortsDaily'],
) {
...
export function YouTubeMusicShortsSongCard({
  item,
  locale,
  chartEndDate,
  fallbackHref,
}: YouTubeMusicShortsSongCardProps) {
  const t = getMessages(locale).youtubeMusicShortsDaily;
