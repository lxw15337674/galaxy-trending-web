...
export async function buildYouTubeMusicDailyShortsSongsPageData(
  rawSearchParams: SearchParamsInput,
  locale: Locale,
): Promise<YouTubeMusicDailyShortsSongsPageData> {
  ...
  try {
    ...
    const snapshot = await getLatestYouTubeMusicDailyShortsSongsSnapshot(country);
    ...
    logServerError('youtube-music/daily-shorts-page-data', error);
